...
def main(cfg: DictConfig) -> None:
    ...
    di = instantiate(cfg.deep_inv.adi, loss={"classes": range(5), "other_classes": range(5, 10)})(
        optimizer=optimizer, teacher=teacher0, student=student, use_amp=cfg.amp, other_teacher=teacher1
    )
    ...
    optimizer = instantiate(cfg.deep_inv.optimizer)([inputs]) 
    di = instantiate(cfg.deep_inv.adi, loss={"classes": range(5, 10), "other_classes": range(5)})(
        optimizer=optimizer, teacher=teacher1, student=student, use_amp=cfg.amp, other_teacher=teacher0
    )
    deep_invs_o.append(di)
    deep_invs.append(di.iterator_from_sampler(sampler))
