...
class StudentTrainer(Trainer):
    ...
    def __post_init__(self) -> None:
        ...
        self.generation_steps = range(
            ...
            self.epoch_gradient_updates // self.new_batches_per_epoch,
        )
        logger.info("Generating new batches at steps %i", str(self.generation_steps))
    ...
    def _train_on_batch(self, images: torch.Tensor, targets: torch.Tensor) -> None:
    ...
    def _train_epoch(self):
        ...
        for step in range(self.epoch_gradient_updates):
            if step in self.generation_steps:
                ...
                logger.info("Student accuracy on new batch: %i", student_acc)
                logger.info("Teacher accuracy on new batch: %i", teacher_acc)
            ...
            epoch_acc += get_batch_accuracy(self.student_net, images, labels)

        epoch_acc /= self.epoch_gradient_updates
        logger.info("Student training acc: %d", epoch_acc)
