...
def main(cfg: DictConfig) -> None:
    ...
    assert len(split) == 2 # This is an experimental run with 2 teachers only
    ...
    teacher0, teacher1 = teachers

    # Di for teacher 0
    sampler = RandomSampler(
        batch_size=cfg.deep_inv.batch_size, classes=range(5)
    )
    inputs = torch.randn(
    ...
    di = instantiate(cfg.deep_inv.di, loss={"classes": range(5), "other_classes": range(5, 10)})(
        optimizer=optimizer, teacher=teacher0, use_amp=cfg.amp, other_teacher=teacher1
    )
    deep_invs.append(di.iterator_from_sampler(sampler))

    # Di for teacher 2
    sampler = RandomSampler(
        batch_size=cfg.deep_inv.batch_size, classes=range(5, 10)
    )
    inputs = torch.randn(
        (cfg.deep_inv.batch_size, *cfg.dataset.input_size),
        requires_grad=True,
    ...
    optimizer = instantiate(cfg.deep_inv.optimizer)([inputs])
    di = instantiate(cfg.deep_inv.di, loss={"classes": range(5, 10), "other_classes": range(5)})(
        optimizer=optimizer, teacher=teacher1, use_amp=cfg.amp, other_teacher=teacher0
    )
    deep_invs.append(di.iterator_from_sampler(sampler))
