...
from fed_distill.deep_inv import DILoss, DeepInversion
...
def main() -> None:
    ...
    # Deep Inversion
    batch_size = 256
    di_loss = DILoss(l2_scale=3e-8, var_scale=2.5e-5, bn_scale=10)
    ...
    di = DeepInversion(di_loss, optimizer, teacher, 2000, True) 
    ...
    if not di_base.load("/mlodata1/jellouli/new_exp0_paper_params/initial_batches.pt"):
        for i in range(50):
            logger.info("Generating initial batch %i", i)
            di_base.grow()
        di_base.save("/mlodata1/jellouli/new_exp0_paper_params/initial_batches.pt")

    # student Training
    student = ResNet18().to("cuda")

    di_dataset = DeepInversionDataset(
        di.iterator_from_sampler(sampler), di_base, CIFAR10_INVERSION_TRANSFORM
    )
    loader = GrowingDatasetDataLoader(di_dataset, 50000, 2, 256, True)
    ...
    di_dataset.save("/mlodata1/jellouli/new_exp0_paper_params/batches.pt")

    torch.save(trainer.metrics, "/mlodata1/jellouli/new_exp0_paper_params/result.pt")
